...
const Header = () => {
  ...
  return (
    <div className="grid justify-items-stretch">
      <div className="justify-self-center text-white font-mono font-bold text-2xl tracking-wide">
        {username}
      </div>
...
const User = () => {
  ...
  useEffect(async () => {
    await axios(`https://api.github.com/users/${username}/repos`).then((it) => {
      setName(it.data)
    })
    return () => {}
  }, [name])
          ...
          return (
            <div key={user.id}>
              ...
              <div className="font-mono font-bold text-green-900">
                Repository URL:
                <Link to={`/${username}/${user.name}`} className="text-green-200">
                  {user.html_url}
                </Link>
              </div>
            </div>
